...
#	@INC = (); # wreck intentionally for testing
	...
	$TTYtter_RC_NUMBER = 3; # non-zero for release candidate
...
"** if your extension is multi-module aware, add it to -exts instead.\n")
		if (length($exts) && (length($olib) || length($lib)));
	if (length($olib)) {
...
"*** shortened to: @{[ (&urlshorten($2) || 'FAILED -- %% to retry') ]}\n";
	...
	# remove it from array, regenerate $track, call tracktags_compile
