...
#	@INC = (); # wreck intentionally for testing
	...
	$TTYtter_RC_NUMBER = 1; # non-zero for release candidate
...
sub csplit { return &cosplit(@_, sub { return &ulength(shift); }); }
# take a string and return up to 140 BYTES plus the rest.
sub usplit { return &cosplit(@_, sub { return   length(shift); }); }
sub cosplit {
